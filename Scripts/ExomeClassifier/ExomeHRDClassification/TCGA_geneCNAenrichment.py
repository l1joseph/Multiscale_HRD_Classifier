...
fishers_gain = pd.DataFrame({'Gene': df.columns[2:], 'Estimate': np.nan, 'pVal': np.nan})
fishers_loss = pd.DataFrame({'Gene': df.columns[2:], 'Estimate': np.nan, 'pVal': np.nan})
...
df_fishers = pd.merge(fishers_gain[['Gene', 'l2fc', 'label']], 
                      fishers_loss[['Gene', 'l2fc', 'label']], 
                      on='Gene', suffixes=('_GAIN', '_LOSS'))
...
df_fishers.loc[df_fishers['Label'] != 'none', 'Gene_Label'] = df_fishers.loc[df_fishers['Label'] != 'none', 'Gene']

# Plot
palette = sns.color_palette("Zissou1", n_colors=5)
g_GainvsLoss = sns.scatterplot(data=df_fishers, x='l2fc_GAIN', y='l2fc_LOSS', hue='Label', palette=palette[:4])
for _, row in df_fishers.dropna(subset=['Gene_Label']).iterrows():
    g_GainvsLoss.text(row['l2fc_GAIN'], row['l2fc_LOSS'], row['Gene_Label'], fontsize=8)
...
plt.savefig('Figures/Supp_GeneCnaEnrich_GainVsLoss.pdf', height=4, width=4, bbox_inches='tight')
plt.close()
