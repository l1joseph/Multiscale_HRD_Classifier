...
# Load HRD/BRCA status
ann_tcga = pd.read_pickle("Data/TCGA_HRDclassification_BRCAannotation_HRD0.79.pkl")
ann_tcga.set_index("Patient", inplace=True)
ann_tcga = ann_tcga.loc[~ann_tcga["BRCA_status"].isna()]
ann_tcga = ann_tcga.loc[~ann_tcga["BRCA_status"].isin(["PALB2", "RAD51C"])]
...
def elastic_net_regression(X, y, weights, alpha=0.25, l1_ratio=0.5, max_iter=1000):
...
# Conduct 250 iterations of 10-fold cross validation
iterations = range(1, 251)
coefs = []
...
for i in iterations:
    ...
    for train_index, val_index in kf.split(input_x):
        ...
        model, scaler = elastic_net_regression(X_train, y_train, weights_train)
        fold_coefs.append(model.coef_)

    coefs.append(np.mean(fold_coefs, axis=0))

# Save coefficients
joblib.dump(coefs, "Results/CVcoefficients_MultiWEN_alpha0.25_p0.79_iter1to250.joblib")
