...
cna = pd.read_csv(
    "~/Data/TCGA/brca_tcga_pan_can_atlas_2018/data_armlevel_cna.txt", sep="\t"
)
...
df_ann = results_tcga_df[["Patient", "HRD"]]
...
for i, arm in enumerate(df.columns[2:]):
    ...
    df_i = pd.DataFrame(
        {"HRD": df["HRD"], "Gain": df[arm_i] == "Gain", "Loss": df[arm_i] == "Loss"}
    )
    ...
    # Gains
    table_gain_i = pd.crosstab(df_i["HRD"], df_i["Gain"])
    ...
    # Losses
    table_loss_i = pd.crosstab(df_i["HRD"], df_i["Loss"])
...
fishers_gain.loc[fishers_gain["padj"] > 0.01, "label"] = np.nan
...
fishers_loss.loc[fishers_loss["padj"] > 0.01, "label"] = np.nan
...
df_fishers.loc[df_fishers["Label"] != "none", "Arm_Label"] = df_fishers.loc[
    df_fishers["Label"] != "none", "Arm"
]
...
sns.scatterplot(
    data=df_fishers, x="l2fc_GAIN", y="l2fc_LOSS", hue="Label", palette=palette[:4]
)
